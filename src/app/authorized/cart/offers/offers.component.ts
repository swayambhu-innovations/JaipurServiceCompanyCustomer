...
export class OffersComponent implements OnInit {
  ...
  ngOnInit() {
    this.booking?.services.forEach((service) => {
      ...
      service.discountsApplicable?.map((discount) => {
        if (
          (discount.type == 'flat' && discount?.value <= this.subTotal) ||
          discount.type != 'flat'
        ) {
    ...
    this.booking?.services.forEach((service) => {
      service.discountsApplicable?.map((discount) => {
        const index = this.discountList.findIndex(
  ...
  searchcoupons() {
    let coopen =
      this.coupons.filter((coupon) => coupon.code == this.searchValue) ||
      undefined;
