...
export class AddressService {
    ...
    collectionData(
      ...
      this.getAddresses2(this.dataProvider.currentUser!.userData.uid,).then(
        (result) => {
          ...
          this.selectedAddres = this.addresses.filter(
  ...
  async getAddresses(userId: string) {
    return (
      await getDocs(collection(this.firestore, 'users', userId, 'addresses'))
    ).docs;
  }
  ...
  deleteAddress(userId: string, addressId: string) {
    return deleteDoc(
      doc(this.firestore, 'users', userId, 'addresses', addressId)
    );
  }
  ...
  getAreaDetail(latitude: number, longitude: number) {
    return this.http.get(
      `${environment.firebase.functionURL}getAreaDetailByLatLng?lat=${latitude}&lng=${longitude}`
    );
  }

  getAreaDetailByPlaceId(placeId: string) {
    return this.http.get(
      `${environment.firebase.functionURL}getAreaDetailByPlaceId?placeId=${placeId}`
    );
  }
